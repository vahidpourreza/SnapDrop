...
import { validateOptionsIntegrity, validateCore, runCustomValidators } from './snapdrop.validation.js';
...
_mergeDefaultOptions(userOptions) {
  ...
  // Perform all checks using validation function
  validateOptionsIntegrity(userOptions);
...
validate() {
  ...
  const coreValidationResult = validateCore(this.state.selectedItems, this.options.limits);
